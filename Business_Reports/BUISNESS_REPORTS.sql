...
CREATE OR REPLACE VIEW V_DRIVER_EARNING
AS
SELECT O.AGENT_ID,D.FIRST_NAME || ' ' || D.LAST_NAME AS NAME,TRUNC( NVL(SUM(O.TIP_TO_AGENT), 0) + NVL(SUM(0.03 * O.ORDER_AMOUNT), 0),3) AS TOTAL_EARNING
FROM ORDERS O
JOIN DELIVERY_AGENT D ON O.AGENT_ID = D.AGENT_ID
GROUP BY O.AGENT_ID, D.FIRST_NAME,D.LAST_NAME ORDER BY TOTAL_EARNING
;
